"""
HGRN-1.3B AWP sparsity sweep: adds AWP results to sparsity_sweep_hgrn.json.

Loads the model once, collects covariance stats, then for each sparsity:
restores dense weights → AWP prune → evaluate PPL.
Writes results into results/sparsity_sweep_hgrn.json under key "awp".

Usage
-----
python scripts/sparsity_sweep_awp_hgrn.py \\
